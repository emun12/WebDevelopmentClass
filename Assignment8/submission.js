...
export async function fn6() {
    ...
    const result = await axios({
        method: 'get',
        url:'https://comp426-1fa20.cs.unc.edu/a08/headers',
        // headers: {'my-custom-response-header': 'hello'},
    }) 

    return result.headers["my-custom-response-header"];
    // for(let i=0; i<result.headers.length; i++){
    //     if(result.headers[i] == "my-custom-response-header"){
           
    //         return result.headers[i].get("my-custom-response-header");
    //         // return result.headers[i]["my-custom-response-header"];
    //         // return result.keys(result.headers[i]);
    //     }
    // }
 };
